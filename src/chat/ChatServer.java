				...
				if (object instanceof ChatMessage) {
					...
					if(message.startsWith("/roll")){
					  try {
  					  ...
  					  int diceSize = Integer.parseInt(message.trim().substring(indexOfD+1));
  					  int min = 1;
  					  
  					  int randomNum = rand.nextInt((diceSize - min) + 1) + min;
  					  chatMessage.text = String.format("%s rolled %d", connection.name, randomNum);
  					}
					  catch (Exception ex){
					    chatMessage.text = "Invalid roll command by " + connection.name;
					  }
					} else {
					  chatMessage.text = connection.name + ": " + message;
					}
					System.out.println("Message: " + chatMessage.text);
